...
public static class SgtProceduralMesh
{
	...
	private static List<Vector2> coords3 = new List<Vector2>();

	private static List<Vector2> coords4 = new List<Vector2>();
	...
	public static void PushCoord3(Vector2 xy)
	{
		coords3.Add(xy);
	}

	public static void PushCoord3(Vector2 xy, int count)
	{
		for (var i = 0; i < count; i++) coords3.Add(xy);
	}

	public static void PushCoord3(float x, float y)
	{
		coords3.Add(new Vector2(x, y));
	}

	public static void PushCoord3(float x, float y, int count)
	{
		var xy = new Vector2(x, y); for (var i = 0; i < count; i++) coords3.Add(xy);
	}

	public static void PushCoord4(Vector2 xy)
	{
		coords4.Add(xy);
	}

	public static void PushCoord4(Vector2 xy, int count)
	{
		for (var i = 0; i < count; i++) coords4.Add(xy);
	}

	public static void PushCoord4(float x, float y)
	{
		coords4.Add(new Vector2(x, y));
	}

	public static void PushCoord4(float x, float y, int count)
	{
		var xy = new Vector2(x, y); for (var i = 0; i < count; i++) coords4.Add(xy);
	}
	...
	public static Mesh SplitSingle(HideFlags hideFlags = HideFlags.None)
	{
		Split(1, hideFlags);

		if (meshes.Count == 1)
		{
			var mesh = meshes[0]; meshes.Clear();
	...
	public static void Split(int verticesStep, HideFlags hideFlags)
	{
		if (positions.Count > 0)
		{
			...
			for (var i = 0; i < meshCount; i++)
			{
				...
				if (coords3.Count > 0)
				{
					mesh.uv = GetRange(coords3, vertA, vertC);
				}

				if (coords4.Count > 0)
				{
					mesh.uv = GetRange(coords4, vertA, vertC);
				}
