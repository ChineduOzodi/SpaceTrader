...
public class NormalGalaxyStarsEffect : MonoBehaviour {
    ...
    public void Update()
    {
        if (GameManager.instance.galaxyView)//MapTogglePanel.instance.solarNormalVisual.isOn || MapTogglePanel.instance.galaxyNormalVisual.isOn)
        {
            if (positions.Count == 0)
            {
                foreach (SolarModel star in game.data.stars)
                {
                    positions.Add(new Vector4((float)(star.solar.referencePosition.x), (float)(star.solar.referencePosition.y),(float)star.solar.referencePosition.z));
            ...
            for (int i = 0; i < game.data.stars.Count; i++)
            {
                ...
                float localScale = (float)Mathd.Pow((game.data.stars[i].solar.bodyRadius), starPow) / starDiv;
